...
// Per-project discovered statuses based on mock data
const discoveredStatusesByProject: Record<string, string[]> = {
  "PROJ": ["To Do", "In Progress", "Blocked", "Done"],
  "WEB": ["To Do", "In Progress", "In Review", "Blocked", "Done"],
  "API": ["To Do", "In Progress", "In Review", "Blocked", "Done"],
  "MOB": ["To Do", "In Progress", "In Review", "Blocked", "Done"],
};

// Default status mappings per project
const defaultStatusMappingsByProject: Record<string, { jiraStatus: string; feedCategory: string }[]> = {
  "PROJ": [
    { jiraStatus: "To Do", feedCategory: "Not Started" },
    { jiraStatus: "In Progress", feedCategory: "In Progress" },
    { jiraStatus: "Blocked", feedCategory: "Blocked" },
    { jiraStatus: "Done", feedCategory: "Done" }
  ],
  "WEB": [
    { jiraStatus: "To Do", feedCategory: "Not Started" },
    { jiraStatus: "In Progress", feedCategory: "In Progress" },
    { jiraStatus: "In Review", feedCategory: "In Progress" },
    { jiraStatus: "Blocked", feedCategory: "Blocked" },
    { jiraStatus: "Done", feedCategory: "Done" }
  ],
  "API": [
...
const mockIssues: Issue[] = [
  {
    projectKey: "PROJ",
    issueKey: "PROJ-100",
    title: "Security & Authentication Epic",
    description: "Epic for all security and authentication related work",
    status: "In Progress",
    ...
    customFields: {},
    statusHistory: [
      { status: "To Do", timestamp: "2026-01-10T09:00:00Z" },
      { status: "In Progress", timestamp: "2026-01-11T10:00:00Z" }
    ],
    comments: ["Tracking all security work for Q1"]
  },
  {
    projectKey: "PROJ",
    issueKey: "PROJ-123",
...
export default function Settings() {
  ...
  // Per-project status mappings
  const [statusMappingsByProject, setStatusMappingsByProject] = useState<Record<string, StatusMapping[]>>(
    defaultStatusMappingsByProject
  );
  const [editingProject, setEditingProject] = useState<string>("");

  // Auto-initialize status mappings for selected projects and auto-select first project
  useMemo(() => {
    // Initialize mappings for any selected projects that don't have them yet
    const updatedMappings = { ...statusMappingsByProject };
    let hasChanges = false;

    selectedProjects.forEach(projectKey => {
      if (!updatedMappings[projectKey]) {
        // Get discovered statuses for this project, or use defaults
        const projectStatuses = discoveredStatusesByProject[projectKey] || discoveredStatuses;
        updatedMappings[projectKey] = projectStatuses.map(status => ({
          jiraStatus: status,
          feedCategory: status === "To Do" ? "Not Started" : 
                       status === "In Progress" ? "In Progress" : 
                       status === "Blocked" ? "Blocked" : 
                       status === "Done" ? "Done" : 
                       status === "In Review" ? "In Progress" : "In Progress"
        }));
        hasChanges = true;
      }
    });
    ...
    if (hasChanges) {
      setStatusMappingsByProject(updatedMappings);
    }

    // Auto-select first project if none selected
    if (selectedProjects.length > 0 && !editingProject) {
      setEditingProject(selectedProjects[0]);
    }
    // Clear selection if current project is no longer selected
    if (editingProject && !selectedProjects.includes(editingProject)) {
      setEditingProject(selectedProjects[0] || "");
    }
  }, [selectedProjects, editingProject]);
  ...
  const updateProjectStatusMapping = (projectKey: string, jiraStatus: string, feedCategory: string) => {
    setStatusMappingsByProject(prev => ({
      ...prev,
      [projectKey]: prev[projectKey]?.map(mapping => 
  ...
  const feedCategoryForIssue = (issue: Issue): string => {
    const projectMappings = statusMappingsByProject[issue.projectKey];
    if (!projectMappings) return "In Progress";
    
    const mapping = projectMappings.find(m => m.jiraStatus === issue.status);
    return mapping?.feedCategory || "In Progress";
  };

  const deriveEpicKey = (issue: Issue): string | undefined => {
    switch (epicStrategy) {
      case "Epic Link":
        return issue.epicKey;
      case "Parent":
      ...
      case "Issue Type Epic":
        ...
        return issue.epicKey || issue.parentKey;
      default:
        return issue.epicKey;
    }
  };
  ...
  const shouldIncludeIssue = (issue: Issue): boolean => {
    // Check stale first
    if (excludeStale && isIssueStale(issue)) return false;
    
    const feedCategory = feedCategoryForIssue(issue);
  ...
  const isValid = useMemo(() => {
    // Check that all selected projects have complete status mappings
    const allProjectStatusesMapped = selectedProjects.every(projectKey => {
      const projectMappings = statusMappingsByProject[projectKey];
      return projectMappings && projectMappings.every(mapping => mapping.feedCategory);
    });
    ...
    return connected && 
           ...
           dueDateFieldValid &&
           teamFieldValid;
  }, [connected, selectedProjects, pilotUsers, statusMappingsByProject, reviewWindowStart, reviewWindowEnd, 
  ...
  const filteredIssues = useMemo(() => {
    ...
    return filtered;
      ...
      requireAssignee, recencyThresholdDays, includeResolvedDays, statusMappingsByProject]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                  <ToggleSwitch
                    ...
                    onChange={setExcludeStale}
                    label="Exclude stale issues"
                  />
                </div>
            ...
            <section className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                <div>
                  <Label htmlFor="editingProject">Editing project:</Label>
                  <Select
                    id="editingProject"
                ...
                {editingProject ? (
                  <div className="space-y-3">
                    {statusMappingsByProject[editingProject]?.map((mapping) => (
                      <div key={mapping.jiraStatus} className="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-700 rounded-lg">
                        ...
                        <Select
                          ...
                          onChange={(e) => updateProjectStatusMapping(editingProject, mapping.jiraStatus, e.target.value)}
                          className="w-40"
                        >
                      ...
                      <p className="text-gray-500 dark:text-gray-400 text-center py-4">
                        No status mappings found for this project.
                      </p>
                    )}
                  </div>
                ) : (
                  selectedProjects.length > 0 ? (
                    <p className="text-gray-500 dark:text-gray-400 text-center py-8">
                      Please select a project above to edit its status mappings.
                    </p>
                  ) : (
                    <p className="text-gray-500 dark:text-gray-400 text-center py-8">
                      Please select projects in the Scope section first.
                    </p>
                  )
                )}
              </div>
                    ...
                    return (
                      <div key={issue.issueKey} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2 text-xs">
                          ...
                          {deriveEpicKey(issue) && (
                            <Badge color="pink">Epic: {deriveEpicKey(issue)}</Badge>
                          )}
